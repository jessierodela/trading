...
export function OpenPositionsTable({ positions }: { positions: PaperPosition[] }) {
  return (
    <div className="bg-[var(--color-surface-card)] border border-[var(--color-border-default)] rounded">
      ...
      {positions.length === 0 ? (
        <p className="px-4 py-5 text-[10px] text-[var(--color-text-muted)]">
          No open paper positions are currently persisted.
        </p>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-left">
            <thead className="text-[8px] uppercase tracking-[.1em] text-[var(--color-text-dim)]">
              <tr>
                {["Symbol", "Direction", "Quantity", "Entry", "Mark", "Unrealized", "Stop", "Target", "Opened", "Strategy", "Risk"].map((heading) => (
            ...
            <tbody>
              {positions.map((position) => (
                <tr key={position.id ?? position.orderId} className="text-[10px] text-[var(--color-text-secondary)]">
                  ...
                  <td className="px-3 py-2 font-mono">{formatPaperCurrency(position.markPrice)}</td>
                  <td className={`px-3 py-2 font-mono ${position.unrealizedPnl >= 0 ? "text-[var(--color-accent-green)]" : "text-[var(--color-accent-red)]"}`}>
                    {formatPaperCurrency(position.unrealizedPnl)}
                  </td>
                  <td className="px-3 py-2 font-mono">{formatPaperCurrency(position.stopLoss)}</td>
                  <td className="px-3 py-2 font-mono">{formatPaperCurrency(position.takeProfit)}</td>
