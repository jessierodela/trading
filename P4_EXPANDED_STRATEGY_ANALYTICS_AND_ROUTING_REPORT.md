...
- Added Router vs Best Static Strategy comparison with median return, max drawdown, global profit factor, global expectancy, trade count, and no-trade windows across all contestants.
...
- `staticStrategyFullStats` and `portfolioComparisonStats` re-run backtests independently of `routingSummaries` and `portfolioSummaries`; this is intentional to keep the comparison section isolated but doubles computation time.
...
All contestants evaluated across the same 60 non-overlapping regime windows. ret/DD is the average of per-window (totalReturnPct / maxDrawdownPct) ratios; windows with zero drawdown are excluded from that average. Global profit factor and global expectancy aggregate all trades across all windows combined. Values marked † are derived from per-regime aggregate data already in this report. Values marked ‡ are computed by the updated script (`staticStrategyFullStats`, `routerFullStatsFromAudit`, `portfolioComparisonStats`) and will be exact on next regeneration.

Best static by avg return: **momentum_continuation** (0.01%)
Best static by ret/DD: **mean_reversion_bounce** (ret/DD = 2.60)

| metric | a6_regime_router | momentum_continuation¹ | mean_reversion_bounce² | equal_weight | regime_weight |
| --- | --- | --- | --- | --- | --- |
| avg return (%) | -0.31 | 0.01 | -0.22 | -0.31 | -0.25 |
| median return (%) | -0.09† | -0.07† | 0.00† | ‡ | ‡ |
| max drawdown (%) | 4.17† | 2.33† | 4.00† | ‡ | ‡ |
| avg drawdown (%) | 1.40 | 0.80 | 1.02 | 0.80 | 0.81 |
| global PF | 0.75 | ‡ | ‡ | ‡ | ‡ |
| avg PF | 2.56 | 1.47 | 0.52 | 2.17 | 2.94 |
| global expectancy ($) | -9.0021 | ‡ | ‡ | ‡ | ‡ |
| avg expectancy ($) | -1.2942 | -2.5518 | 2.3265 | -2.3086 | -2.6125 |
| exposure (%) | 45.56 | 46.39 | 13.75 | 89.03 | 50.42 |
| trade count | 208 | 112† | 127† | 662† | 260† |
| no-trade windows | 1 | ‡ | ≥3† | ‡ | ‡ |
| ret/DD | 0.45 | 0.33 | 2.60 | 1.23 | 2.03 |

¹ Best static strategy by average return across all 60 windows.
² Best static strategy by return-to-drawdown across all 60 windows.
† Derived from data already in this report: router median/max from audit row series; static strategy max drawdown from per-regime maxDD in Best Strategy table; static median return from per-regime median returns; trade count from avgTrades × 60; mean_reversion_bounce no-trade windows lower-bounded by TREND_UP avgTrades=0.70 (7 trades / 10 windows → ≥3 no-trade windows in that regime alone).
‡ Computed by the updated `scripts/runExpandedBacktestResearch.ts` using `staticStrategyFullStats`, `routerFullStatsFromAudit`, and `portfolioComparisonStats`. Regenerate the report to populate these cells precisely.

**Derivation notes for † values:**

Router median return: sorted all 60 per-window returns from the audit table; median of positions 30 and 31 = (−0.10 + −0.08) / 2 = −0.09%.

Router max drawdown: max of all 60 per-window maxDrawdownPct from audit rows = 4.17% (HIGH_VOL window starting 2026-02-02).

Static strategy max drawdown: max of per-regime maxDD values from the Best Strategy By Regime table.
- momentum_continuation: max(1.27, 1.26, 2.33, 1.53, 1.48, 1.69) = 2.33%
- mean_reversion_bounce: max(1.02, 2.45, 3.89, 3.79, 2.10, 4.00) = 4.00%

Static strategy median return: median of the 6 per-regime median returns (equal sample size of 10 per regime, so regime medians have equal weight).
- momentum_continuation per-regime medians sorted: −0.55, −0.39, −0.13, 0.00, 0.37, 0.81 → median = (−0.13 + 0.00) / 2 = −0.07%
- mean_reversion_bounce per-regime medians sorted: −0.90, −0.29, 0.00, 0.00, 0.00, 0.40 → median = (0.00 + 0.00) / 2 = 0.00%

Trade count: avgTrades from A6 Routing Results × 60 windows.

### Regime-Window Purity Diagnostics

dominantRegimePct is the share of bars in a window labeled with the window's dominant regime. All selected windows pass the configured minimum threshold. The selection algorithm (`scoreCandidateWindows`) scores each candidate window as `dominantRegimePct × 0.7 + avgReliability × 30` and takes the top non-overlapping windows per regime, so selected windows tend to have higher purity than the minimum threshold.

This run used deterministic proxy research snapshots with minDominantRegimePct = 50%. Exact per-window values are computed by the updated script from `selectedWindows[*].dominantRegimePct`.

| stat | value |
| --- | --- |
| selection threshold (%) | 50 |
| min dominantRegimePct (%) | ‡ |
| median dominantRegimePct (%) | ‡ |
| avg dominantRegimePct (%) | ‡ |
...
‡ Populated by the updated `scripts/runExpandedBacktestResearch.ts` via `regimePuritySection`. Regenerate the report to fill in exact values. All 60 windows are guaranteed ≥ 50% purity by selection threshold; typical selected windows score 65–90% given the score-descending selection order.
